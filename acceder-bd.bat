...
docker start mongodb-sistema-contable 2>nul || docker run -d --name mongodb-sistema-contable -p 27017:27017 mongo:latest

echo.
echo 4. VERIFICANDO CONEXIÓN...
timeout /t 3 /nobreak >nul
docker exec mongodb-sistema-contable mongosh --eval "db.runCommand('ping')" 2>nul
if %errorlevel% equ 0 (
